...
class AV1Transport {
    constructor(client, stream, details) {
        this.rtpPackets = [];
        this.waitingForSequenceHeader = true; // used when writing .obu file as 'ffplay' does not like it if the first OBUs are not TD then SH
        this.client = client;
        this.stream = stream;
        // process 'fmtp'
    ...
    processRTPFrame(rtpPackets) {
        const obus = []; // the OBUs from the RTSP server, which normally come without their length bytes (leb128) 
        for (let i = 0; i < rtpPackets.length; i++) {
            ...
            */
            ...
            while (ptr < packet.length) {
                ...
                if (aggregation_header_w == 0)
                    hasLeb128Prefix = true;
                if (aggregation_header_w != 0 && obuCount != aggregation_header_w)
                    hasLeb128Prefix = true;
                ...
                if (obu.length > 0) {
                    ...
                    */
                    ...
                    if (size_bit == 0) {
                        let size = 0;
                        if (extension_bit == 0)
                            size = obu.length - 1; // -1 for the OBU header
                        if (extension_bit == 1)
                            size = obu.length - 2; // -2 for the OBU header and the Header Extension Byte
...
exports.default = AV1Transport;
//# sourceMappingURL=AV1Transport.js.map