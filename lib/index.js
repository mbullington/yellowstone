...
class RtspClient extends EventEmitter {
  ...
  */
  ...
  connect(url, options) {
    ...
    return this._netConnect(hostname, port)
      ...
      .then(obj => {
        ...
        // channel 0, RTP
        // channel 1, RTCP
        return this.request("SETUP", { Transport: `${RTP_AVP}/TCP;interleaved=0-1` });
      })
      .then(headers => {
        const transport = parseTransport(headers.Transport);
        
        if (transport.protocol !== `${RTP_AVP}/TCP`) {
          throw new Error(
            `Only RTSP servers supporting ${RTP_AVP} over TCP are supported at this time.`
          );
        }
      ...
      const responseHandler = (responseName, headers, mediaHeaders) => {
        ...
        if (statusCode === STATUS_OK) {
          ...
          if (statusCode === STATUS_UNAUTH) {
            ...
            const authHeaders = {};
            {
              const match = WWW_AUTH_REGEX.exec(headers[WWW_AUTH]);
              while (match) {
      ...
      if (this.readState == ReadStates.SEARCHING && data[index] == PACKET_START) {
        ...
        if (this.rtspPacketPointer == this.rtspPacketLength) {
          ...
          if (packetChannel === 1) {
            ...
            this._sendEmptyReceiverReport(packetChannel);
          }
          this.readState = ReadStates.SEARCHING;
        }
      // read response data
  ...
  _sendEmptyReceiverReport(channel) {
    ...
    this._sendInterleavedData(channel, report);
  }
}
